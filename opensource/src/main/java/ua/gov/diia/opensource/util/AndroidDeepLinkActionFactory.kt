...
import ua.gov.diia.opensource.helper.documents.DocName.DRIVER_LICENSE
...
class AndroidDeepLinkActionFactory @Inject constructor() :
            ...
            if (checkDocParams.size > 1) {
                ...
                if (docType != null) {
                    return DeepLinkActionViewDocument(
                        ...
                        resourceId = checkDocParams.getOrLogEvent(3),
                        notificationId = checkDocParams.getOrLogEvent(4),
                    )
                }
            }
        }
    ...
    override fun buildPathFromPushNotification(pushNotification: PushNotification): String {
        return when (PushNotificationActionType.fromId(pushNotification.action.type)) {
            PushNotificationActionType.MESSAGE -> { "$DIIA_HTTPS$DEEP_LINK_MESSAGE${pushNotification.needAuth}/${pushNotification.notificationId ?: Preferences.DEF}/${pushNotification.action.resourceId?:""}" }
    ...
    private fun List<String>.getOrLogEvent(index: Int): String {
