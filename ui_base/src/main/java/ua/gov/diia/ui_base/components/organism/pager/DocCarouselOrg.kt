...
data class DocCarouselOrgData(val data: SnapshotStateList<DocsCarouselItem>, val focusOnDoc: Int? = null) : UIElementData {
    ...
    fun flipCard(index: Int): DocCarouselOrgData {
        return if (index < data.size) {
            this.data[index] = if (this.data[index] is DocCardFlipData) {
                (this.data[index] as DocCardFlipData).changeCardSide()
            } else {
                data[index]
            }
            this
    ...
    fun flipAllCardsToFrontSideIfNeeded(): DocCarouselOrgData {
        return if (data.any { it is DocCardFlipData && it.currentSide != CardFace.Front }) {
            val newData = SnapshotStateList<DocsCarouselItem>().apply {
                data.forEach { docCarouselItem ->
                    add(
                        when (docCarouselItem) {
                            is DocCardFlipData -> {
                                if (docCarouselItem.currentSide != CardFace.Front) {
                                    docCarouselItem.setCardSide(cardSide = CardFace.Front)
                                } else {
                                    docCarouselItem
                                }
                            }

                            else -> docCarouselItem
                        }
                    )
                }
            }
