...
class DocGalleryVMComposeTest {
    ...
    private lateinit var vm: DocGalleryVMCompose

    private fun initVM() {
        vm = DocGalleryVMCompose(
            apiDocs,
    ...
    fun `test do init`() = runTest {
        ...
        vm.doInit(settings)
        Assert.assertEquals(settings, vm.settings)
    }

    @Test
    fun `test default settings`() = runTest {
        initVM()
        advanceUntilIdle()
        Assert.assertEquals(DocFSettings.default, vm.settings)
    }
    ...
    fun `test TOGGLE_BUTTON_MOLECULE qr action`() = runTest {
        ...
        vm.docAction.test {
            ...
            advanceUntilIdle()
            Assert.assertEquals(
                DocGalleryVMCompose.DocActions.DefaultBrightness,
                awaitItem()
            )
        }
    }
    ...
    fun `test TOGGLE_BUTTON_MOLECULE ean action`() = runTest {
        ...
        vm.docAction.test {
            ...
            advanceUntilIdle()
            Assert.assertEquals(
                DocGalleryVMCompose.DocActions.HighBrightness,
                awaitItem()
            )
        }
    }
    ...
    fun `test DOC_ELLIPSE_MENU action`() = runTest {
        ...
        vm.navigation.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.Navigation.ToDocActions(
                    DocTest(),
                    0,
                    null
                )
    ...
    fun `test TICKER_ATOM_CLICK action`() = runTest {
        ...
        vm.navigation.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.Navigation.NavToVehicleInsurance(
                    DocTest()
                )
            )
        }
    ...
    @Test
    fun `test ADD_DOC_ORG action`() = runTest {
        Mockito.`when`(documentsDataSource.data).thenReturn(documentsFlow2)
        val manualDocs = ManualDocs(emptyList())
        Mockito.`when`(apiDocs.getDocsManual()).thenReturn(manualDocs)
        initVM()
        advanceUntilIdle()
        val settings = DocFSettings()
        vm.doInit(settings)
        vm.navigation.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.Navigation.ToDocActions(
                    mockDoc.diiaDocument!!, 0, manualDocs
                )
            )
        }
    }
    ...
    @Test
    fun `test DOC_STACK action`() = runTest {
        initVM()
        advanceUntilIdle()
        val settings = DocFSettings()
        vm.doInit(settings)
        vm.navigation.test {
            vm.onUIAction(
                UIAction(
                    actionKey = UIActionKeysCompose.DOC_STACK,
                    data = "doc_test"
                )
            )
            val item = awaitItem()
            assert(item is DocGalleryVMCompose.Navigation.ToDocStack)
            Assert.assertEquals(
                (item as DocGalleryVMCompose.Navigation.ToDocStack).doc,
                DocTest()
            )
        }
    }
    ...
    @Test
    fun `test DOC_STACK data null action`() = runTest {
        initVM()
        advanceUntilIdle()
        val settings = DocFSettings()
        vm.doInit(settings)
    ...
    @Test
    fun `test DOC_STACK doc null action`() = runTest {
        initVM()
        advanceUntilIdle()
        val settings = DocFSettings()
        vm.doInit(settings)
    ...
    fun `test DOC_ACTION_IN_LINE action`() = runTest {
        ...
        vm.docAction.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.DocActions.OpenElectronicQueue
            )
        }
    }
    ...
    fun `test DOC_ACTION_TO_DRIVER_ACCOUNT action`() = runTest {
        ...
        vm.docAction.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.DocActions.OpenDriverAccount
            )
        }
    }

    @Test
    fun `test CHANGE_DOC_ORDER  action`() = runTest {
        initVM()
        vm.navigation.test {
            vm.onUIAction(
                UIAction(
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.Navigation.ToDocStackOrder
            )
        }
    }
    ...
    fun `test DOC_NUMBER_COPY  action`() = runTest {
        ...
        vm.docAction.test {
            ...
            Assert.assertEquals(
                awaitItem(),
                DocGalleryVMCompose.DocActions.DocNumberCopy("")
            )
        }
    }
